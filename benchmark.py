...
class EvaluationMetrics:
    ...
    tokens_per_layer: Dict[str, Metric]
    ...
    def update(
        ...
        for metric in self.tokens_per_layer.values():
            if generation_result.generation_strategy_result.tokens_per_layer is None:
                tokens_per_layer = torch.tensor(0)
            else:
                tokens_per_layer = torch.tensor(
                    generation_result.generation_strategy_result.tokens_per_layer
                )
            metric.update(tokens_per_layer)
    ...
    def compute(self) -> Dict[str, torch.Tensor]:
        return {
            ...
            "tokens_per_layer": {
                metric_name: metric.compute().item()
                for metric_name, metric in self.tokens_per_layer.items()
            },
        }
...
def benchmark(
    ...
    elif generation_config.generation_strategy == "DELE_speculative":
    ...
    init_num_speculations = generation_config.num_speculations
    for i, example in enumerate(tqdm(evaluation_set)):
        # reset num_speculations for next input for FSM speculation.
        if generation_config.generation_strategy == "FSM_speculative":
            generation_config.num_speculations = init_num_speculations
        ...
        if response.generation_strategy_result.tokens_per_layer is not None:
            print(f"[TPL]: {response.generation_strategy_result.tokens_per_layer}")
...
def main(args: Arguments, benchmark_arguments: BenchmarkArguments, generation_config: GenerationConfig, output_fname: str):
    ...
    # Get the maximum memory allocated during the process
    max_mem_allocated = torch.cuda.max_memory_allocated(device) / (1024 ** 2)  # In MB
    max_mem_reserved = torch.cuda.max_memory_reserved(device) / (1024 ** 2)    # In MB

    print(f"Max memory allocated: {max_mem_allocated:.2f} MB")
