...
# Implements Draft&Verify method
class DVSpeculativeGenerationStrategy(GenerationStrategy):
    def __init__(self):
        super().__init__()
        self.AR = 0
        self.beta_1 = 0.5
    ...
    def generate_token_ids(
        ...
        while len(output_ids) < generation_config.max_steps:
            ...
            total_layers += generation_config.exit_layer * num_speculations + len(model.model.layers)
            
            accr = total_draft_matches / total_generations
            if self.AR == 0:
                self.AR = accr
            else:
                self.AR = self.beta_1 * self.AR + (1-self.beta_1) * accr
            
            new_threshold = self.cs_threshold
            if self.AR < self.alpha:
                new_threshold += self.epsilon
            else:
                new_threshold -= self.epsilon
            self.cs_threshold = self.beta_2 * self.cs_threshold + (1-self.beta_2) * new_threshold
    ...
    def single_step_speculation(
        ...
        for di in range(d_max):
            ...
            draft_next_prob = draft_next_prob[0, draft_next_token].item()
            if draft_next_prob < self.cs_threshold:
                break
